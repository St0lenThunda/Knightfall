  ...
  <div class="page play-page">
    ...
    <div class="play-layout" :class="{ 'game-active': !showSetup }">
      ...
      <div class="side-panel glass">
        ...
        <!-- TACTICAL PULSE -->
        <div class="tactical-pulse" v-if="store.mode === 'vs-computer' && gameActive">
          <!-- Phase 1: Mood Orb -->
        ...
        <div class="divider" v-if="store.mode === 'vs-computer' && gameActive"></div>
...
// Watch FEN for analysis trigger, blunder detection, and pass & play camera flipping
watch(() => store.fen, (newFen) => {
  if (gameActive.value) {
    if (store.mode === 'vs-computer') {
      ...
      // Blunder detection: check eval after engine settles (Phase 2)
      setTimeout(() => {
        ...
        // Only alert on the player's own moves
      ...
      }, 1500) // Wait for engine to reach reasonable depth
...
const renderedBlunder = computed(() => {
  if (!blunderText.value) return ''
  return marked.parse(blunderText.value, { async: false }) as string
})

// Phase 3: Explain
const isExplaining = ref(false)
const explainText = ref<string | null>(null)
const renderedExplain = computed(() => {
  if (!explainText.value) return ''
  return marked.parse(explainText.value, { async: false }) as string
})
...
async function handleExplain() {
  ...
  // Pause the clock while studying
  if (clockInterval) { clearInterval(clockInterval); clockInterval = null }

  try {
    const lastSan = store.moveHistory.length > 0 ? store.moveHistory[store.moveHistory.length - 1].san : '...'
  ...
  } catch { explainText.value = 'Unable to analyze position.' }
...
function dismissExplain() {
  explainText.value = null
  // Resume the clock
  if (gameActive.value && !clockInterval) startClock()
}
